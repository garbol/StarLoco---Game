...
public class Main {
    ...
    public static String loginHostDB = "127.0.0.1", loginNameDB = "local", loginUserDB = "root", loginPassDB = "", loginPortDB = "3306";
    //Game
                    ...
                    if(!Config.getInstance().HEROIC) {
                        ...
                        for (GameMap map : World.world.getMaps()) {
                            for (Monster.MobGroup group : map.getMobGroups().values()) {
                                if(!group.isFix())
                                    Database.getDynamics().getHeroicMobsGroups().insert(map.getId(), group, null);
                            }
                        }
                    }
                    GameServer.setState(0);
    ...
    public static void start() {
        ...
        Main.logger.debug("Current timestamp ns : " + System.nanoTime()  + " " + Long.parseLong("44720239120550", 10));
