    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            if(this.fighter.getCurPm(this.fight) > 0 && L == null && C == null) {
                int nbrcase = Function.INSTANCE.getmoveautourIfPossible(this.fight, this.fighter, ennemy);
                if(nbrcase != 0) {
                    time = nbrcase;
                    action = true;
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && !action) {
                if (Function.INSTANCE.getHealIfPossible(this.fight, this.fighter, false, 80) != 0) {
                    time = 1000;
                    action = true;
                }
            }
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && C != null && !action) {
                int value = Function.INSTANCE.getattackIfPossibleRN(this.fight, this.fighter, C);
                if(value != 0) {
                    time = value;
                    action = true;
                }
