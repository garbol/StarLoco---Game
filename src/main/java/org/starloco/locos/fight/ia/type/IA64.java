    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && L == null && C == null) {
                int value = Function.INSTANCE.getattackBondIfPossible(this.fight, this.fighter, ennemy);
                if(value != 0) {
                    time = value;
                    action = true;
                    L = Function.INSTANCE.getgetNearestEnnemynbrcasemax(this.fight, this.fighter, 1, maxPo + 1);// pomax +1;
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && L != null && C == null && !action) {
                int value = Function.INSTANCE.getattackIfPossible(fight, this.fighter, this.highests);
                if(value != 0) {
                    time = value;
                    action = true;
                }
